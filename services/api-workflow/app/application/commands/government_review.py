...
def review_context(actor:Actor,queue_id:str)->dict:
    ...
    return {"queueId":queue_id,"status":row[0],"invoiceVersionId":row[1],"packageId":row[2],"zipArtifactId":row[3],"packageHashes":row[4],"configurationVersionId":row[5],
            "configurationVersion":{"kind":"configuration","id":row[5],"version":row[11],"sha256":canonical_hash(row[12])},"documentProfiles":row[13],
            "invoiceVersion":row[6],"amount":str(row[7]),"ngo":row[8],"contract":row[9],"submittedAt":row[10].isoformat(),"validation":{"id":run[0],"engineVersion":run[1],"inputHash":run[2],"outputHash":run[3]},"findings":[{"code":x[0],"severity":x[1],"message":x[2],"status":x[3],"expenseKey":x[4]} for x in findings],"artifacts":[{"path":x[0],"artifactId":x[1],"sha256":x[2],"mediaType":x[3]} for x in artifacts],"provenance":[{"eventType":x[0],"actorId":x[1],"occurredAt":x[2].isoformat(),"payload":x[3]} for x in events]}
