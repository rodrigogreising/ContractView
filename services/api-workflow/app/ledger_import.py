...
def import_ledger(job: IngestionJob, artifact: Artifact) -> dict:
    ...
    with database() as connection:
        existing = connection.execute(
            "select id,row_count,control_total,imported_total from ledger_imports where artifact_id=%s", (artifact.id,)
        ).fetchone()
        if existing:
            return {"id": existing[0], "rowCount": existing[1], "controlTotal": str(existing[2]), "importedTotal": str(existing[3])}
        config = connection.execute(
            "select payload from configuration_versions where contract_id=%s order by version desc limit 1", (job.contract_id,)
        ).fetchone()
        ...
        connection.execute(
            """insert into ledger_imports
               (id,job_id,artifact_id,contract_id,organization_id,source_sheet,importer_version,schema_version,mapping_version,control_total,imported_total,row_count)
        ...
        for row_number, row, cells in rows:
            ...
            connection.execute(
                """insert into expense_rows
                   (id,ledger_import_id,contract_id,organization_id,expense_key,expense_date,vendor,description,budget_category,amount,invoice_number,evidence_filename,
                    source_artifact_id,source_sheet,source_row,source_cells,importer_version,schema_version,mapping_version)
