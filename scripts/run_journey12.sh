...
MODE="${1:-headless}"
OUTPUT="${2:-artifacts/journey12/${MODE}-$(date -u +%Y%m%dT%H%M%SZ)}"
PROJECT="${JOURNEY12_PROJECT_NAME:-${CI_PROJECT_NAME:-contractview}-journey12}"
PORT="${JOURNEY12_WEB_PORT:-14173}"
...
finish() {
  ...
  if [[ "${JOURNEY12_KEEP_RUNTIME:-false}" != "true" ]]; then
...
if [[ "$MODE" == "headed" ]]; then
  JOURNEY12_BASE_URL="http://127.0.0.1:${PORT}" \
  JOURNEY12_ARTIFACT_DIR="$(pwd)/$OUTPUT" \
  JOURNEY12_HEADED=true \
  JOURNEY12_SLOW_MO_MS="${JOURNEY12_SLOW_MO_MS:-650}" \
