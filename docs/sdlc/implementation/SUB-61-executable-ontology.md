...
All registries are version `1.0.0`. The dependency graph is acyclic. A minor
version may add optional fields; closed-vocabulary changes, removals,
required-field changes, or semantic changes require a major version and
...
- Registry validator: 14 required contracts, semantic versions, lifecycle
  parity, unique closed vocabularies, and acyclic dependencies pass.
- Compatibility tests prove optional additions pass and closed-vocabulary
  changes fail.
- Five pure Python Pydantic tests validate round trips, hashes, unknown-field
  rejection, bundles, events, views, templates, rules, entities, relations, and
  the existing identity response shape.
- Local TypeScript evidence: 2 files / 11 tests pass; `tsc` and Vite build pass.
...
- Corrected clean-container evidence explicitly rebuilt `api` and `web-test`
  before execution: 156 API tests and 11 frontend tests pass after a
  deterministic reset.
...
Immutable artifact hashes, PR manifest, four AI reviews, merge SHA, and
post-merge verification are pending. The issue must not move to In Review or
Done until those checks complete.
